from . import ctrl, esc, cursor, screen, rendition
	...
	def select_gfx(self, *params):
		...
		while i < lp:
			...
			if p == 0:
				...
				self.gfx &= ~(0x00FF0000 | rendition.GFX_FG)
				self.gfx |= ((p - 29) << 16)
			elif p == 38 and i + 2 < len(params):
				if params[i+1] == 5:
					self.gfx &= ~0x00ff0000
					self.gfx |= rendition.GFX_FG
					self.gfx |= (params[i+2] & 0xff) << 16
					i = i + 2
			elif p == 39:
				self.gfx &= ~(0x00ff0000 | rendition.GFX_FG)
			elif p >= 40 and p <= 47:
				self.gfx &= ~(0xFF000000 | rendition.GFX_BG)
				self.gfx |= ((p - 39) << 24)
			elif p == 48 and i + 2 < len(params):
				if params[i+1] == 5:
					self.gfx &= ~0x00ff0000
					self.gfx |= rendition.GFX_BG
